...
3. **The API blocks its own event loop.** `supabase-py` is synchronous; ~all of the 160+ async handlers call it directly. One slow query stalls every in-flight request on that worker. **(✅ Fixed — `aexecute` seam + all 112 async-handler `.execute()` sites converted, ratcheted by a CI guard.)**
4. **Background throughput is capped at one job at a time.** `WORKER_CONCURRENCY=1` on a single FIFO queue → head-of-line blocking, zero per-tenant fairness; one tenant's batch starves everyone. **(✅ Mostly fixed — latency-class queue split + per-tenant in-flight cap; horizontal worker autoscale remains.)**
...
## 2a. ✅ Shipped since this review (Phase 1 + Phase 2)

| Area | What shipped | PRs |
|------|--------------|-----|
| **Tenant safety (Phase 1)** | Composite-key tenancy (global `UNIQUE`s → `(user_id, …)`), per-user JWT auth through the proxy, `user_id` scoping on all `server.py` tenant endpoints, RLS-scoped per-request client, AST guardrail test for unscoped queries. | #163, #164 |
| **Async data layer (Phase 2, P0 #3)** | Additive `db/client.py::aexecute` seam (threadpool-offloaded, bounded) + **all 112 blocking `.execute()` in async handlers** converted across `server.py` + 6 routers, behind an AST CI guard (`tests/test_no_blocking_execute_in_async.py`) that fails the build on any new loop-blocking call. | #165, #171–#175 |
| **Queue fairness (Phase 2, P1)** | Latency-class queue split + **per-tenant in-flight cap** (Redis counter, flag-gated, fail-open) so one tenant's batch can't starve others. | #167 |
| **Bounded fan-out (Phase 2, P1)** | `agents/concurrency.py::gather_bounded`; the 3 unbounded per-row `asyncio.gather` LLM/HTTP fan-outs are now semaphore-bounded. | #166 |
| **Observability + recovery** | Observability core + `/ready` readiness probe, orphan-reaper fix, Redis circuit breaker, cost rollup + HNSW, dismissals FK. | #164-era |

**Single-user prod is byte-for-byte unchanged** by all of the above — every change is behaviour-preserving or flag-gated default-off.
...
- **✅ Sync DB blocks the event loop — FIXED (Phase 2, #165 + #171–#175).** Added the additive `db/client.py::aexecute` seam (runs the sync `supabase-py` `.execute()` in a bounded threadpool) and converted **all 112 blocking `.execute()` sites inside async handlers** across `server.py` + the 6 routers to `await aexecute(...)`. An AST CI guard (`tests/test_no_blocking_execute_in_async.py`) now fails the build if a loop-blocking `.execute()` is reintroduced in an async function. Behaviour-preserving; single-user prod unchanged.
...
- **Worker throughput ceiling + no fairness — ✅ mostly fixed (#167).** Latency-class queue split + a **per-tenant in-flight cap** (Redis counter, flag-gated, fail-open) landed. **Remaining:** horizontal worker autoscale on queue depth — an infra/Railway scaling decision, not code. *(M / med.)*
...
- **Cron batch jobs run in-process, serially — 📋 deferred (build note).** The queue-side `AsyncIOScheduler` in `api/queue.py` (scout / validator / persona / g6 `CronTrigger` jobs) executes full graphs on the scheduler's event loop (O(tenants×apps) sequential LLM calls; lost on redeploy). **Fix:** each scheduled job should only *enqueue* per-tenant RQ jobs — which already carry the #167 in-flight cap — instead of running graphs inline; the worker pool then absorbs the load fairly. **Deferred because** it only matters once there are multiple tenants to fan out across, and the change is best verified against the live queue. *(M / med.)*
...
- **Connection pooler — 📋 deferred (action: config now, code later).** The `supabase-py` REST client doesn't hold raw PG connections, but the **LangGraph HITL checkpointer** opens a direct `:5432` connection per graph run via `AsyncPostgresSaver.from_conn_string` at 3 sites (`agents/g7_io.py:327`, `agents/g9_graph.py:85`, `resume_agents/g2_graph.py:144`). Concurrent runs across replicas/workers can exhaust Postgres `max_connections`.
  - **Do now (zero code, zero risk):** point `SUPABASE_DB_URL` at the Supabase **session-mode pooler** host (`...pooler.supabase.com:5432`). `from_conn_string` keeps working unchanged (session mode supports prepared statements); connections are reused instead of opened per run. Recommended immediate step before raising concurrency.
  - **Do later (code, staging-gated):** for maximum multiplexing use the **transaction pooler `:6543`** with a shared `psycopg_pool.AsyncConnectionPool` (kwargs `prepare_threshold=None`, `autocommit=True`) passed to `AsyncPostgresSaver`, replacing per-run `from_conn_string`. **Deferred because** `langgraph-checkpoint-postgres` is a prod-only dependency (absent from the dev/test env), so this path can't be verified without a staging deploy — and it only pays off at concurrent multi-tenant load. Build it when a staging environment + real tenant concurrency exist. *(M / med.)*
...
- **LangGraph hygiene. ✅ Fixed (Phase 1).** G2/G3 checkpoint `thread_id`s are now tenant-namespaced with `user_id` (`g2-{user_id}-job-{job_id}`, `g3-{user_id}-app-{application_id}-r{n}`) so two tenants sharing a `job_id`/`application_id` no longer collide on one checkpoint thread; G9 was already user-namespaced. Every graph `ainvoke` now passes an explicit `recursion_limit` (G2=50, G3/G6=40, G4/G9=25) so a routing bug raises a controlled `GraphRecursionError` instead of running to LangGraph's silent default of 25. In single-user mode `user_id` is the seed UUID, so thread_ids stay deterministic and behaviour is unchanged. *(G7's `thread_id`, keyed on `application_answer_id` only, plus G8/G11 `ainvoke`s, are the same class and remain to do — out of this PR's scope.)*
...
**Phase 2 — Throughput & isolation (do before real concurrent load).** ✅ Async data layer (`aexecute` seam + full conversion + CI guard), ✅ queue split + per-tenant fairness, ✅ bounded fan-outs. **Remaining:** worker autoscale (infra); connection pooler (**session-pooler config now**, transaction-pooler code staging-gated); per-tenant spend caps; externalize circuit breaker + rate limiter to Redis. Per-tenant spend caps are the highest-value remaining item and gate **billing/margin** (Phase 3).
